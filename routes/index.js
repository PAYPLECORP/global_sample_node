...
router.get('/', (req, res) => {
    const defaultData = {
        ...
        service_oid: "PaypleGpayTest-" + createOid(),
        ...
        resultUrl: process.env.HOSTNAME + "/node/result"
    }

    res.render('order', defaultData);
});


// POST /order_confirm, 결제창 호출 페이지 렌더링(order_confirm.html)
router.post('/order_confirm', (req, res) => {
    res.render('order_confirm', req.body);
});

// POST /order_billingKey, 빌링키 결제 페이지 렌더링(order_billingKey.html)
router.post('/order_billingKey', (req, res) => {
    res.render('order_billingKey', req.body);
});
...
router.post('/result', (req, res) => {
    ...
    res.render('order_result', responseData);
})
...
router.post('/cancel', async (req, res) => {
    try {
        ...
        const cancelData = {
            ...
            resultUrl: process.env.HOSTNAME + "/node/result"    // [선택] 파트너사에서 취소 성공시 리다이렉트 하는 등 활용할 수 있는 파라미터입니다.
        }
